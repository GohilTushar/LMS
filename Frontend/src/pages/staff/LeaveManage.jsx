...
import { approveLeaveAPI, getLeaveReportAPI, rejectLeaveAPI } from "../../service/ApiCall";
...
const LeaveManageTable = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await getLeaveReportAPI(page,sortField,sortOrder,status)
      ...
      <table className="min-w-full bg-white border rounded-lg shadow-lg">
        <thead className="bg-blue-900 text-white">
          <tr>
            ...
            <th className="py-6 px-4 cursor-pointer border uppercase">Action</th>
          </tr>
        </thead>
        <tbody>
          {loading
            ? renderSkeleton()
            :leaveList.length>0? leaveList.map((leave) => (
                <tr key={leave.id} className="odd:bg-blue-50 even:bg-white hover:bg-indigo-100 transition-colors duration-300">
                  <td className="px-6 py-4 border whitespace-nowrap text-gray-700">{leave.requestedBy.name}</td>
                  <td className="px-6 py-4 border whitespace-nowrap text-gray-700">{leave.requestedBy.email}</td>
                  <td className="px-6 py-4 border whitespace-nowrap text-gray-700">{leave.requestedBy.department}</td>
                  <td className="px-6 py-4 border whitespace-nowrap text-gray-700">{leave.requestedBy.phone}</td>
                ...
                <td colSpan={9} className="p-5 border text-center">No Data Founded</td>
              )
              }
        </tbody>
      </table>
